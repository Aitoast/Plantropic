...
// events.routes.js — 사용자별 일정 CRUD (Express)
// 모든 라우트는 requireAuth 로 보호되어 req.userId(로그인 사용자) 기준으로만 데이터 접근.
...
router.use(requireAuth); // ← 여기서 로그인 상태를 확인. 미인증이면 401.

// 목록: ?from=<ISO>&to=<ISO> (기간 필터, starts_at 기준)
...
router.post("/", h(async (req, res) => {
  ...
  if (!title?.trim() || !startsAt || !endsAt)
    return res.status(400).json({ message: "제목과 시간은 필수예요." });
  ...
  if (!cal) return res.status(400).json({ message: "존재하지 않는 캘린더예요." });
...
router.patch("/:id", h(async (req, res) => {
  ...
  if (!Object.keys(fields).length) return res.status(400).json({ message: "변경할 내용이 없어요." });
...
router.delete("/:id", async (req, res) => {
  ...
  if (!row) return res.status(404).json({ message: "일정을 찾을 수 없어요." });
  res.json(serialize(row));
}));

router.delete("/:id", h(async (req, res) => {
  const ok = await db.deleteEvent(req.userId, req.params.id);
  if (!ok) return res.status(404).json({ message: "일정을 찾을 수 없어요." });
  res.status(204).end();
});
}));
