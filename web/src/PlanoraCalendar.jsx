...
//   const evByDay = useMemo(() => groupByDay(calOn), [calOn]);
...
//         <Sidebar {...{ y, m, selDay, setSelDay, calOn, toggleCal, periodLabel, prevPeriod, nextPeriod }} />
...
//         </main>
//         {selected && <DetailPanel event={selected} onClose={() => setSelEventId(null)} />}
//       </div>
//     </div>
//   );
// }
...
//         <span style={{ fontSize: 19, fontWeight: 600, letterSpacing: '-0.4px' }}>Planora</span>
//       </div>
...
//         </div>
//         <h1 style={{ fontSize: 20, fontWeight: 600, letterSpacing: '-0.5px', margin: '0 0 0 8px', minWidth: 150 }}>{periodLabel}</h1>
//       </div>
//       <div style={{ flex: 1 }} />
...
// // ---- Sidebar ------------------------------------------------------
// function Sidebar({ y, m, selDay, setSelDay, calOn, toggleCal, periodLabel, prevPeriod, nextPeriod }) {
...
//       <button className="pl-create" style={{ height: 46, border: 'none', background: c.ink, color: '#fff', borderRadius: 11, fontFamily: 'inherit', fontSize: 14.5, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '0 1px 2px rgba(0,0,0,0.15)' }}>
...
//           <button onClick={onClose} style={{ width: 30, height: 30, border: 'none', background: 'rgba(255,255,255,0.55)', borderRadius: 8, cursor: 'pointer', color: c.textMuted, fontSize: 15, flexShrink: 0 }}>✕</button>
//         </div>
//         <h2 style={{ fontSize: 21, fontWeight: 700, letterSpacing: '-0.5px', margin: '14px 0 0', lineHeight: 1.25 }}>{event.title}</h2>
//       </div>
...
//           <MetaRow icon="◔"><div style={{ fontSize: 14, color: c.textInk }}>{event.reminder}</div></MetaRow>
//         </div>

//         <Divider />

//         <div>
//           <SectionLabel>{`참석자 · ${event.attendees.length}`}</SectionLabel>
//           <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
//             {event.attendees.map(([name, status], i) => (
...
//         <Divider />
//         <div><SectionLabel>메모</SectionLabel><p style={{ fontSize: 13.5, lineHeight: 1.6, color: c.textMuted, margin: 0 }}>{event.desc}</p></div>
//         <Divider />

//         <div>
//           <SectionLabel>{`댓글 · ${event.comments.length}`}</SectionLabel>
//           <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
//             {event.comments.map(([who, time, text], i) => (
...
export default function PlanoraCalendar() {
  ...
  const { events, addEvent, updateEvent, deleteEvent } = useEvents(EVENTS);
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        ...
        {selected && (
          <DetailPanel 
            ...
            onDelete={() => { deleteEvent(selected.id); setSelEventId(null); }}
          />
        )}
      </div>
